function openTab(evt, tabName, speed) {
  var container = ".menu__tabs";
  var buttonsContainer = ".menu__nav";
  var buttons = ".menu__button";
  var content = ".menu__content";
  var tabNameArr = [];
  ...
  if ($(evt.target).closest(container).find(content).is("#" + tabName)) {
    ...
    if (evt.target.classList[0] === "menu__button") {
...
$(".menu__tabs").each(function() {
  $(this).find(".menu__button:first").trigger("click");
});